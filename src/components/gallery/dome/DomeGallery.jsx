...
const DEFAULT_IMAGES = [
  ...
  },
  {
    ...
    alt: 'Karya seni digital',
  },
  {
    src: 'https://images.unsplash.com/photo-1755353985163-c2a0fe5ac3d8?q=80&w=774&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    alt: 'Seni kontemporer',
  },
  {
  ...
  },
  {
    ...
    alt: 'Permukaan bertekstur',
  },
  { src: 'https://pbs.twimg.com/media/Gyla7NnXMAAXSo_?format=jpg&name=large', alt: 'Gambar media sosial' },
]
...
export default function DomeGallery({
  ...
  grayscale = true,
  autoRotateSpeedDegPerSecond = DEFAULTS.autoRotateSpeedDegPerSecond,
  autoRotateResumeDelayMs = DEFAULTS.autoRotateResumeDelayMs,
}) {
  ...
  const autoRotateLastFrameRef = useRef(null)
  ...
  const effectiveImages = images?.length ? images : DEFAULT_IMAGES
  ...
  const pauseAutoRotate = useCallback((durationMs = autoRotateResumeDelayMs) => {
    autoRotatePausedUntilRef.current = performance.now() + durationMs
    autoRotateLastFrameRef.current = null
  ...
  useEffect(() => {
    if (autoRotateSpeedDegPerSecond === 0) return undefined
    ...
    const tick = (now) => {
      const isPaused =
        ...
        inertiaRAF.current ||
        now < autoRotatePausedUntilRef.current

      if (isPaused) {
        autoRotateLastFrameRef.current = now
        autoRotateRAF.current = requestAnimationFrame(tick)
        return
      }

      if (autoRotateLastFrameRef.current == null) {
        autoRotateLastFrameRef.current = now
      } else {
        const elapsedSeconds = (now - autoRotateLastFrameRef.current) / 1000
        autoRotateLastFrameRef.current = now
        const nextY = wrapAngleSigned(rotationRef.current.y + autoRotateSpeedDegPerSecond * elapsedSeconds)
  ...
  }, [enlargeTransitionMs, pauseAutoRotate, unlockScroll])
  ...
  const openItemFromElement = useCallback(
    ...
    [enlargeTransitionMs, lockScroll, openedImageHeight, openedImageWidth, pauseAutoRotate, segments, unlockScroll],
  )
